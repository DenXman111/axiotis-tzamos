...
int main() {
    auto lookup = [](unsigned int j,unsigned int i, vector<int>& a, vector<int>& b, int k, int p, int qq, int ww) {
        return -matrix[i][j];
    };
    vector<int> a;
    vector<int> argmax = smawk<int>(NUM_COLS, NUM_ROWS, lookup, a, a, 0, 0, 0, 0);
    for (int i = 0; i < argmax.size(); ++i) {
        cout << i << " : " << argmax[i] << " " << lookup(i, argmax[i], a, a, 0, 0, 0, 0) << endl;
    }
    return 0;
}